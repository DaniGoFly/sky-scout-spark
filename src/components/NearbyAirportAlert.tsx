/**
 * NearbyAirportAlert — subtle insider-tip banner shown when a nearby
 * departure airport offers meaningfully cheaper fares (≥ €40 savings).
 *
 * Design: calm, professional, non-pushy. No flashy colors.
 */
...
interface NearbyAirportAlertProps {
  ...
  /** All flights including from nearby origins (multi-origin mode) */
...
// Approximate drive times (minutes) from common hubs to nearby alternatives
...
function formatDriveTime(minutes: number): string {
  if (minutes < 60) return `${minutes}m drive`;
  const h = Math.floor(minutes / 60);
  const m = minutes % 60;
  return m > 0 ? `${h}h ${m}m drive` : `${h}h drive`;
}

const SAVINGS_THRESHOLD = 40; // minimum savings in original currency to show alert
...
const NearbyAirportAlert = memo(({
  ...
  const suggestion = useMemo(() => {
    ...
    // Look for alternate origins in allFlights
    ...
    for (const f of allFlights) {
      ...
      if (existing === undefined || f.price.amount < existing) {
        alternates.set(src, f.price.amount);
      }
    }

    // Find best alternate that saves >= threshold
      ...
      if (savings >= SAVINGS_THRESHOLD) {
        if (!bestAlt || savings > bestAlt.savings) {
          bestAlt = {
            ...
            price,
            savings,
            driveTime: getDriveTime(currentOrigin.toUpperCase(), origin),
          };
        }
      }
    }
    ...
    <div className="relative flex items-start gap-3 rounded-xl border border-primary/15 bg-primary/5 px-4 py-3.5 text-sm">
      ...
      <div className="flex-1 min-w-0 pe-6">
        <p className="font-semibold text-foreground leading-snug">
          {suggestion.origin} saves {savingsLabel}
        </p>
        <p className="text-muted-foreground text-xs mt-0.5 leading-relaxed">
          {suggestion.origin} → {destination} from {priceLabel}
          {suggestion.driveTime !== null && (
            <span> · Approx. {formatDriveTime(suggestion.driveTime)} from {currentOrigin}</span>
          )}
        </p>
      </div>
