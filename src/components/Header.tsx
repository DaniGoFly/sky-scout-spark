...
const Header = () => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-[hsl(var(--hero-bg))]/95 backdrop-blur-xl border-b border-white/10" style={{ paddingTop: "env(safe-area-inset-top)" }}>
      <div className="container mx-auto px-4 min-h-[56px] h-16 flex items-center justify-between max-w-full box-border overflow-hidden">
        ...
        <nav className="hidden md:flex items-center gap-1">
          {isHome ? (
            <span className="px-4 py-2 rounded-lg text-sm font-medium text-white bg-white/15 cursor-default">
              {t("nav.flights")}
            </span>
          ) : (
            <Link
